...
{
    LIST_HEAD( blocked_users.users_list );
}

void init_blocked_groups(void)
{
    LIST_HEAD( blocked_groups.groups_list );
}
...
void destroy_blocked_users(void)
{
    ...
    list_for_each_safe(cur_head, temp_head, &blocked_users.users_list )
    {
...
void destroy_blocked_groups(void)
{
    ...
    list_for_each_safe(cur_head, temp_head, &blocked_groups.groups_list )
    {
...
void destroy_blocked_globs(void)
{
    ...
    list_for_each_safe(cur_head, temp_head, &blocked_globs.globs_list )
    {
...
bool in_blocked_users( uid_t uid )
{
    ...
    list_for_each_entry( tmp, &blocked_users.users_list, list)
    {
        if ( tmp->uid == uid )
        {
            res = true;
...
bool in_blocked_groups( gid_t gid )
{
    ...
    list_for_each_entry( tmp, &blocked_groups.groups_list, list )
    {
        if ( tmp->gid == gid )
        {
            res = true;
...
bool in_blocked_paths( char* path )
{
    ...
    list_for_each_entry( tmp, &blocked_globs.globs_list, list )
    {
...
bool block_user( uid_t uid )
{
	...
	list_add( &(tmp_user->list), &blocked_users.users_list );
    return true;
}
...
bool block_group( gid_t gid )
{
	...
	list_add( &(tmp_group->list), &blocked_groups.groups_list );
    return true;
}
...
bool block_path( char* path )
{
	...
	list_add( &(tmp_glob->list),  &blocked_globs.globs_list );
    return true;
}
